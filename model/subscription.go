...
func CompleteSubscriptionOrderWithPaymentCheck(tradeNo string, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string, paidMoney string) error {
	return completeSubscriptionOrder(tradeNo, providerPayload, expectedPaymentProvider, actualPaymentMethod, paidMoney, true)
}

func completeSubscriptionOrder(tradeNo string, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string, paidMoney string, strictPaymentCheck bool) error {
	if tradeNo == "" {
		return errors.New("tradeNo is empty")
	}
	var paid decimal.Decimal
	var err error
	if strictPaymentCheck {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if strictPaymentCheck {
			...
			expected := decimal.NewFromFloat(order.Money).Round(2)
			if !paid.Equal(expected) {
				return fmt.Errorf("subscription payment amount mismatch: expected=%s actual=%s", expected.StringFixed(2), paid.StringFixed(2))
			}
		}
