...
const localTokenEstimateMultiplier = 1.1

func applyLocalTokenEstimateMultiplier(tokens int) int {
	if tokens <= 0 {
		return tokens
	}
	return int(math.Ceil(float64(tokens) * localTokenEstimateMultiplier))
}
...
func EstimateRequestToken(c *gin.Context, meta *types.TokenCountMeta, info *relaycommon.RelayInfo) (int, error) {
	...
	tkm = applyLocalTokenEstimateMultiplier(tkm)
...
func CountTokenRealtime(info *relaycommon.RelayInfo, request dto.RealtimeEvent, model string) (int, int, error) {
	...
	textToken = applyLocalTokenEstimateMultiplier(textToken)
	audioToken = applyLocalTokenEstimateMultiplier(audioToken)
