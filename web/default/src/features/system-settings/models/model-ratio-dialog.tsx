...
export function ModelRatioDialog({
  ...
  const [pricingSubMode, setPricingSubMode] = useState<PricingSubMode>('price')
    ...
    if (editData) {
      ...
      if (editData.billingMode === 'tiered_expr') {
        ...
        if (editData.ratio) {
          ...
          if (editData.cacheRatio) {
            setCachePrice(
              (tokenPrice * parseFloat(editData.cacheRatio)).toString()
            )
          }
          if (editData.createCacheRatio) {
            setCreateCachePrice(
              (tokenPrice * parseFloat(editData.createCacheRatio)).toString()
            )
          }
        }
      }
  ...
  const handlePromptPriceChange = (value: string) => {
    ...
    if (value && !isNaN(parseFloat(value))) {
      const prompt = parseFloat(value)
      form.setValue('ratio', (prompt / 2).toString())
      if (completionPrice && !isNaN(parseFloat(completionPrice))) {
        form.setValue(
          'completionRatio',
          (parseFloat(completionPrice) / prompt).toString()
        )
      }
      if (cachePrice && !isNaN(parseFloat(cachePrice))) {
        form.setValue('cacheRatio', (parseFloat(cachePrice) / prompt).toString())
      }
      if (createCachePrice && !isNaN(parseFloat(createCachePrice))) {
        form.setValue(
          'createCacheRatio',
          (parseFloat(createCachePrice) / prompt).toString()
        )
      }
    } else {
      form.setValue('ratio', '')
      ...
      form.setValue('createCacheRatio', '')
    }
  }

  const handleRelativeTokenPriceChange = (
    value: string,
    field: 'completionRatio' | 'cacheRatio' | 'createCacheRatio',
    ...
    if (
      ...
      form.setValue(
        field,
        (parseFloat(value) / parseFloat(promptPrice)).toString()
      )
    } else {
      form.setValue(field, '')
    }
                    ...
                    <div className='space-y-4'>
                      ...
                      <div className='space-y-2'>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleRelativeTokenPriceChange(
                              e.target.value,
                              'completionRatio',
                              setCompletionPrice
                            )
                      ...
                      <div className='space-y-2'>
                        <Label>{t('Cache read price ($/1M tokens)')}</Label>
                        <Input
                          type='text'
                          placeholder='0.5'
                          value={cachePrice}
                        ...
                        <p className='text-muted-foreground text-sm'>
                          {cachePrice &&
                          !isNaN(parseFloat(cachePrice)) &&
                          promptPrice &&
                          !isNaN(parseFloat(promptPrice)) &&
                          parseFloat(promptPrice) > 0
                            ? t('Calculated ratio: {{ratio}}', {
                                ratio: (
                                  parseFloat(cachePrice) /
                                  parseFloat(promptPrice)
                                ).toFixed(4),
                              })
                            : t('Enter cache read price to calculate ratio')}
                        </p>
                      </div>

                      <div className='space-y-2'>
                        <Label>{t('Cache write price ($/1M tokens)')}</Label>
                        <Input
                          type='text'
                          placeholder='2.0'
                        ...
                        <p className='text-muted-foreground text-sm'>
                          {createCachePrice &&
                          !isNaN(parseFloat(createCachePrice)) &&
                          promptPrice &&
                          !isNaN(parseFloat(promptPrice)) &&
                          parseFloat(promptPrice) > 0
                            ? t('Calculated ratio: {{ratio}}', {
                                ratio: (
                                  parseFloat(createCachePrice) /
                                  parseFloat(promptPrice)
                                ).toFixed(4),
                              })
                            : t('Enter cache write price to calculate ratio')}
                        </p>
                      </div>
                    </div>
                  </>
